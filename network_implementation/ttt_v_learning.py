...
from TicTacToe.players import QNetPlayer, RandomPlayer, VNetPlayer
...
net = FullyConnectedNet([18, 20, 20, 1], nonLin=Tanh())
folder = "networks/ttt_v_learning_other_player_exploit/"
...
memory = ReplayMem(10, 1, net, gamma=0.95, encode=TTTVEncoding(), goal_value_function=VGoal())
...
for epoch in range(epochs):
    ...
    batcher = SimpleBatcher(batch_size, memory.get_v_data())
    test_batcher = batcher.subset_percent(max([0.2, 1.0 / len(batcher)]))
    ...
    if epoch % evaluation_epochs == 0:
        ...
        avrg_q_batcher = SimpleBatcher(10, eval_mem.get_v_data())
...
print(player1.rand/player1.moves)
print(player0.rand/player0.moves)
